...
from src.config import ARM_COLOR, EE_RADIUS, LINK_COLLISION_THRESHOLD, LINK_COLLISION_WEIGHT

class ArmNR:
    """
    A multi-link arm that computes forward kinematics and solves inverse kinematics.
    The IK solver is used internally to drive the arm so that the end-effector (EE)
    reaches a desired target (which is also the action space for diffusion training).
    ...
    def compute_joint_positions(self) -> list:
        """
        Compute (x, y) positions for each joint (including the EE) using forward kinematics.
        """
    ...
    def compute_joint_positions_np(self, angles: np.ndarray) -> list:
        """
        Compute joint positions given a NumPy array of angles (used for IK optimization).
        """
        positions = []
    ...
    def solve_ik(self, target: torch.Tensor) -> float:
        """
        Solve inverse kinematics to drive the EE toward the target using a least-squares optimizer.
        """
